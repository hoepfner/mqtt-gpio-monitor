...
if MODULE.lower() == "wiringpi":
    try:
        import wiringpi as wiringpi
        logging.info("WiringPi module detected...")
        WIRING_MODULE = True
    except ImportError:
        logging.error("Module = %s in %s but WiringPi module was not found" % (MODULE, INIFILE))
        sys.exit(2)
...
def on_message(mosq, obj, msg):
    ...
    if WIRING_MODULE:
        if pin not in GPIO_OUTPUT_PINS:
            wiringpi.pinMode(pin, 1) # set to Output
...
def cleanup(signum, frame):
    ...
    if WIRING_MODULE:
        logging.debug("Clean up WIRINGPI module")
...
def init_wiringpi():
    ...
    """
    #wiringpi.wiringPiSetupGpio() # GPIO Port numbers
    #wiringpi.wiringPiSetupGpio() # pin numbers
    wiringpi.wiringPiSetupPhys() # Physical P1 header pin numbers  
    # fuer mpc23017
    # Q: http://raspi.tv/2013/using-the-mcp23017-port-expander-with-wiringpi2-to-give-you-16-new-gpio-ports-part-3
    # Q: https://github.com/WiringPi/WiringPi-Python/blob/master/examples/two-mcp23017.py
    # pin_base = 65
    # i2c_addr = 0x20
    # i2c_addr_2 = 0x21
...
def read_pin(pin):
    ...
    if WIRING_MODULE:
        ...
        logging.debug("WiringPi Read GPIO input pin %d..." % (pin))
